...
def render_session_diff(data: dict) -> str:
    return _render_session_diff_steps(data)
...
def _render_session_diff_steps(data: dict) -> str:
    meta = data["meta"]
    ...
    session_id = str(meta.get("id", ""))
    branch = str(meta.get("branch", "?"))
    files_changed = data.get("files_changed", len(files))
    steps_changed = data.get("steps_changed", len(steps))
    ...
    if not steps:
        out.append(f"  {C.DIM}No file-changing steps found in this session.{C.RESET}")
        out.append("")
        out.append(_hr())
        out.append("")
        return "\n".join(out)
    ...
    out.append(
        f"  {C.BOLD}{steps_changed}{C.RESET} step(s)"
        f"  {C.BOLD}{files_changed}{C.RESET} file(s)"
        f"  {C.BGREEN}+{total_added}{C.RESET}"
        f"  {C.BRED}-{total_removed}{C.RESET}"
    )
    out.append("")
    out.append(_hr("─"))
    out.append("")
    ...
    for step in steps:
        ...
        added = step.get("lines_added", 0)
        removed = step.get("lines_removed", 0)
        stats = ""
        if added:
        ...
        out.append(
            f"  {C.BLUE}{step_id.replace('step_', '')[:12]}{C.RESET}"
            f"  {C.BOLD}{tool_or_type}{C.RESET}{stats}"
        )
        for f in step.get("files") or []:
            fp = f.get("file_path", "?")
            action = f.get("action", "write")
            source = f.get("diff_source", "recorded")
            out.append(f"    {C.DIM}{action}{C.RESET} {fp} {C.DIM}{source}{C.RESET}")
            for h in (f.get("hunks") or [])[:4]:
                out.append(f"      {C.DIM}@@ -{h.get('from', 0)} +{h.get('to', 0)} @@{C.RESET}")
            if f.get("git_diff") and not f.get("hunks"):
                out.append(f"      {C.DIM}git diff fallback available in the TUI/json output{C.RESET}")
        out.append("")

    out.append(_hr())
    out.append(
        f"  {C.DIM}{steps_changed} step(s)  ·  {files_changed} file(s)  ·  +{total_added} -{total_removed}  ·  mach diff <id> --json{C.RESET}"
    )
    out.append("")
    return "\n".join(out)
